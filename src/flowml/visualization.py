...
from flowml.config import (
    FAULT_CLASSES,
    INSTANCE_FIGURES_DIR,
    KEY_SENSORS,
    RAW_DATA_DIR,
    SIGNATURE_FIGURES_DIR,
    SOURCE_TYPES,
...
# A variable whose range is this small relative to its own level counts as
# flat on a signature page. Real 3W instances are full of frozen variables
# (9.77 % of them, per Table 7 of the paper) and of readings that drift by a
# few float digits; left to autoscale, both fill their axis with amplified
# noise that reads like a signal. See ``_plot_signature``.
SIGNATURE_FLAT_SPAN = 1e-4
...
def _plot_signature(
    ...
    for i, variable in enumerate(variables):
        ...
        delta, center = top - bottom, 0.5 * (bottom + top)
        note = ""
        if delta <= SIGNATURE_FLAT_SPAN * max(abs(center), 1.0):
            # A frozen or barely moving variable is held at the middle of a
            # padded axis, so it reads as the flat line it is rather than as
            # autoscaled noise, and its ticks state the level it sits at.
...
def plot_fault_signatures(
    ...
    out_path = source_dir / f"fault_{fault_class}_{source}_signatures.pdf"
    ...
    with PdfPages(out_path) as pdf:
        for i, filepath in enumerate(files, start=1):
            ...
            fig = _plot_signature(env, fault_name, filepath.name, variables, units, subtitle)
            pdf.savefig(fig)
            plt.close(fig)
