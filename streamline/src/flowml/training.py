...
def load_task_data(filter_type: str, task: str) -> TaskData:
    """Load the features parquet and assemble the dataset for one task.

    Parameters
    ----------
    filter_type : str
        Which features parquet to read (``"gaussian"``, ``"statistical"``,
        ``"none"``).
    task : str
    ...
    """
    path = features_path(filter_type)
    if not path.exists():
        raise FileNotFoundError(
            f"{path} not found. Build it first:\n"
            f"  uv run scripts/01_build_features.py --filter {filter_type}"
        )
...
def oof_predict(
    ...
    for fold, (train_idx, test_idx) in enumerate(
        gkf.split(data.X, y_enc, data.groups), start=1
    ):
