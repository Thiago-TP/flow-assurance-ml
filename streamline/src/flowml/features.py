...
def extract_instance_features(
    ...
    The instance is z-scored, then each window of each sensor is filtered and
    reduced to 11 statistics. Windows whose 3W ``class`` column is entirely NaN
    ...
    filter_type : str
        Signal filter applied inside each window: ``"gaussian"``,
        ``"statistical"``, or ``"none"``.
    ...
    for start in range(0, len(df) - window_size + 1, step_size):
        ...
        for sensor in sensors:
            filtered = apply_filter(sensor_arrays[sensor][start:end], filter_type)
            row.update(window_features(filtered, sensor))
        rows.append(row)
...
def build_features(
    ...
    filter_type : str
        Signal filter: ``"gaussian"``, ``"statistical"``, or ``"none"``.
    raw_dir : Path
    ...
    try:
        ...
        for fault_class, df_raw in iter_raw_instances(
            ...
            df_feat = extract_instance_features(df_clean, filter_type)
            del df_clean
            if df_feat.empty:
                continue
